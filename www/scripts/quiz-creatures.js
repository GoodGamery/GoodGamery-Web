...
var resultOptions = [HUMAN_RESULT, HUMAN_RESULT, HUMAN_RESULT, HUMAN_RESULT, HUMAN_RESULT];
...
function calcResults(totalScore) {
    ...
    if (quizSteps.find('.active').length == quizSteps.length){
        ...
        // populate results
        var titleText = "<h1>" + resultOptions[n].title + "</h1>";
        var descText = "<p class='desc'>" + resultOptions[n].desc + "</p>";
        console.log("Setting result to " + titleText + ": " + descText);
        resultsTitle.replaceWith(titleText);
        resultsDesc.replaceWith(descText);
        return;
    }
}