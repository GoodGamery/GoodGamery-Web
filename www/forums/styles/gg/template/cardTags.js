...
function getImageSource(cardname, set)
{
    ...
    cardname = cardname.replace( /[\xE8\xE9]/g, "e");          //'e
...
function timeto(to) {
	...
	var rv = '';
	rv += years ? years + ' years, ' : '';
	rv += weeks ? weeks + ' weeks, ' : '';
	rv += hours ? hours + ' hours, ' : '';
	rv += minutes ? minutes + ' minutes, ' : '';
	rv += seconds ? seconds + ' seconds' : '';
                 ...
                 function( inner )
                 {
                     ...
                     for (var bnum = 0; bnum < blocks.length; bnum++) {
                         ...
                         for( var ix = 0; ix < lines.length; ix++ )
                         {
                             ...
                             {
                                 out_text += '<a href="/includes/mtg/mtg_helper_cardfinder.php?find=' + parsed.name + '&width=200&height=285" class="jTip" name=""  onclick=\'window.open("http://magiccards.info/card.php?card=' + parsed.name + '")\' >' + parsed.name + '</a><br/>';
